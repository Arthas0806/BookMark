...
-- 2. 보안이 필요한 컬럼의 테이처를 보호하기 위해

-- create or replace view {뷰 이름}
-- as {sub query}

-- 자중 사용되는 30번 부서에 소속된 사원들의
-- 사버ㄴ과 이름과 부서번호를 출력하기 위한
...
from(select * from emp order by hiredate)    -- 인라인 뷰 : 가사으이 테이블을 만든다.
where rownnum >= 5
;
