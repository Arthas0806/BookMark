...
select ename from emp where substr(hiredate, 4, 2) = '04'; -- 00/00/00

--18. MOD 함수를 사용하여 사원번호가 짝수인 사람만 출력하시오.
select mgr from emp where mod(mgr, 2) = 0;
...
select job, min(sal) as "최저금액" from emp  where mgr is not null group by job having not(min(sal) < 2000) order by min(sal) desc;
